...
def bagging(model_name, train_vecs, train_labels, valid_vecs, valid_labels, save_path):
    ...
    for e in tqdm(range(train_times), desc="Bagging Training"):
        print(e)
        _, samp_vec, _, samp_labels = train_test_split(train_vecs, train_labels, test_size=sample_rate, random_rate=888)
        if model_name == "dtree":
            model = DTree()
        else:
            pass

        train_score = model.train(samp_vec, samp_labels)
        print(train_score)
    ...
    return mean_squared_error(valid_labels, bagging_preds) ** 0.5
